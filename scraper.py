...
from lxml import html as LH  # XPath用
...
IR_HTML= "https://irbank.net/{code}"                          # （英字付きもページはあるが構造差あり：フォールバック用途）
...
def get_csv(code, path):
    # IRBANK CSVは基本「数字4桁」のみ安定。英字付きは試さずスキップ。
...
# Kabutan overview
def kabu_per(code):   return _num_only(_get_first_text_by_xpath(KABU_OVERVIEW.format(code=code), "//th[contains(.,'PER')]/following-sibling::td[1]"))
def kabu_pbr(code):   return _num_only(_get_first_text_by_xpath(KABU_OVERVIEW.format(code=code), "//th[contains(.,'PBR')]/following-sibling::td[1]"))
def kabu_roe_pct(code): return _num_only(_get_first_text_by_xpath(KABU_OVERVIEW.format(code=code), "//th[contains(.,'ROE')]/following-sibling::td[1]"))
def kabu_divy_pct(code):return _num_only(_get_first_text_by_xpath(KABU_OVERVIEW.format(code=code), "//th[contains(.,'配当利回り')]/following-sibling::td[1]"))
def kabu_credit(code):
    t = _get_first_text_by_xpath(KABU_OVERVIEW.format(code=code), "//*[contains(text(),'信用倍率')][1]/following::text()[1]") \
        or _get_first_text_by_xpath(KABU_OVERVIEW.format(code=code), "//th[contains(.,'信用倍率')]/following-sibling::td[1]")
    return _num_only(t)
# Kabutan finance
def kabu_equity_ratio_pct(code):
    return _num_only(_get_first_text_by_xpath(KABU_FINANCE.format(code=code), "//th[contains(.,'自己資本比率')]/following-sibling::td[1]"))
...
def main():
    ...
    for i,code in enumerate(filtered,1):
        ...
        # CSVで計算できたらそれを使う。だめならHTMLで直接比率を拾う
        ...
        # ===== HTMLフォールバック（Sheetsと同じXPath）=====
        # PER/PBR/ROE/配当/信用倍率/自己資本比率は、Kabutanで数値そのものを取得可能
        if per=="":     per = kabu_per(code) or ""  # PERは独立指標なのでOK
